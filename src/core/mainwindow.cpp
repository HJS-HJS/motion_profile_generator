...
void MainWindow::createDocks() {
    ...
    m_nodeEditGroup->setEnabled(false); // Initially disabled

    rightLayout->addStretch(); // Push groups to the top
    ...
    m_yDivisionsSpin->setRange(1, 10); // Min 1 (+/- 1), Max 10 (+/- 10)
    m_yDivisionsSpin->setValue(10);    // Default +/- 10 divisions
    m_yDivisionsSpin->setToolTip("Number of divisions above (and below) the X-axis");
    gridLayout->addRow("Y Divisions (+/-):", m_yDivisionsSpin);

    m_refYSpin = new QDoubleSpinBox; // Ref Y Spinbox
    m_refYSpin->setRange(1.0, 100000.0); // Min 1
    m_refYSpin->setDecimals(1);
    m_refYSpin->setValue(100.0); // Default 100
    ...
    QGroupBox* rangeGroup = new QGroupBox("X Range (ms)");
    QFormLayout* rangeLayout = new QFormLayout;
    m_xMinSpin = new QDoubleSpinBox;
    m_xMinSpin->setRange(-1000000.0, 1000000.0); // Allow negative X min
    m_xMinSpin->setDecimals(0);
    m_xMinSpin->setValue(-100.0); // Default to -100
    m_xMinSpin->setSingleStep(100.0);
    m_xMaxSpin = new QDoubleSpinBox;
    m_xMaxSpin->setRange(0.0, 1000000.0); // Keep Max >= 0 for now
    m_xMaxSpin->setDecimals(0);
    m_xMaxSpin->setValue(2000.0); // Default 2 seconds
    m_xMaxSpin->setSingleStep(100.0);
    rangeLayout->addRow("X Min:", m_xMinSpin);
    rangeLayout->addRow("X Max:", m_xMaxSpin);
    rangeGroup->setLayout(rangeLayout);
    viewLayout->addWidget(rangeGroup);
...
void MainWindow::onApplyViewSettings() {
    ...
    // m_view->setReferenceXDuration(...); // Removed

    // Apply X Range
    if (m_xMaxSpin->value() > m_xMinSpin->value()) {
        m_view->setXRange(m_xMinSpin->value(), m_xMaxSpin->value());
    } else {
        QMessageBox::warning(this, "Invalid Range", "X Max must be greater than X Min.");
    }
}
    ...
    if (!m_document->loadFromYAML(fileName)) {
        ...
        statusBar()->showMessage("YAML file loaded.", 3000);
         QTimer::singleShot(0, this, [=]() {
             loadViewSettings(); // Load saved settings first
             // Apply settings from docks
             onApplyViewSettings();
         });
    }
}
...
void MainWindow::onActiveMotorSwitched(MotorProfile* active, MotorProfile* previous) {
    ...
    if (active) {
        ...
        // Apply View Options X Range on motor switch
        onApplyViewSettings(); // Use the slot
...
void MainWindow::saveViewSettings() {
    ...
    // settings.setValue("ReferenceX", m_refXSpin->value()); // Removed
    settings.setValue("XMin", m_xMinSpin->value());
    settings.setValue("XMax", m_xMaxSpin->value());
...
void MainWindow::loadViewSettings() {
    ...
    m_refYSpin->setValue(settings.value("ReferenceY", 100.0).toDouble());
    // m_refXSpin->setValue(settings.value("ReferenceX", 1000.0).toDouble()); // Removed
    m_xMinSpin->setValue(settings.value("XMin", -100.0).toDouble());
    m_xMaxSpin->setValue(settings.value("XMax", 2000.0).toDouble());
    ...
    if(m_view) {
        m_view->setNumYDivisions(m_yDivisionsSpin->value());
        m_view->setReferenceYValue(m_refYSpin->value());
        // m_view->setReferenceXDuration(m_refXSpin->value()); // Removed
